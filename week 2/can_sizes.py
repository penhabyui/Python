...
def main():
    best_storage_effieciency = 0
    best_cost_effieciency = 0

    for can in get_can_list():
        volume = compute_volume(can['radius'], can['height'])
        surface_area = compute_surface_area(can['radius'], can['height'])
        storage_efficiency = volume / surface_area
        cost_efficiency = volume / can['cost']

        if(storage_efficiency > best_storage_effieciency):
            best_storage_effieciency = storage_efficiency

        if(cost_efficiency > best_cost_effieciency):
            best_cost_effieciency = cost_efficiency

        print(f"{can['name']} \t\t {storage_efficiency:,.2f} \t {cost_efficiency:,.2f}")

    print(f"Best storage efficiency is: {best_storage_effieciency:,.2f}")
    print(f"Best cost efficiency is: {best_cost_effieciency:,.2f}")
